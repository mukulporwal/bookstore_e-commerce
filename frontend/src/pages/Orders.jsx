      ...
      <div>
        {
          orderData.slice().map((item, index)=> ( 
            <div key={index} className='py-4 border-t border-b border-black text-blue-700 flex flex-col md:flex-row md:items-center md:justify-between gap-4'>
              <div className='flex items-start gap-6 text-sm'>
                <img className='w-16 sm:w-20' src={ backendUrl + `${item.image}`} alt="" />
                <div>
                  <p className='sm:text-base font-medium'>
                    {item.name}
                  </p>
                  <div className="flex items-center gap-3 mt-1  text-base text-blue-700">
